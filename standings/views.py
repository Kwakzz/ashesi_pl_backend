...
def get_team_no_of_fa_cup_group_stage_matches_played(season_id, team_id, gender):
    ...
    no_of_fa_cup_group_matches_played = Match.objects.filter(
        Q(match_day__season=season, competition=competition, home_team=team, stage=stage, has_ended=True) |
        Q(match_day__season=season, competition=competition, away_team=team, stage=stage, has_ended=True)
    ).count()
